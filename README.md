...
Same `tts_daemon` for all — each adapter runs a post-reply integration when a reply finishes. [Claude setup](docs/adapters/claude.md) · [Codex](docs/adapters/codex.md) · [OpenCode](docs/adapters/opencode.md) · [Generic template](docs/adapters/generic.md)
...
| **Codex** | Restart Codex if needed so the Aftertone plugin loads → trust the Aftertone hook in `/hooks` if prompted |
...
**`/aftertone-on`** and **`/aftertone-off`** (Claude: `/aftertone_on` / `/aftertone_off`; Codex/OpenCode: `python -m aftertone on|off`, or the Aftertone MCP tools) apply to **this chat only**. Run the matching control in each session where you want speech; other sessions stay silent until you enable them there too. After enabling, send any agent reply so the hook can register that session.
