...
4. **User-facing control:** Cursor slash commands in [`.cursor/commands/`](../.cursor/commands/) only — they call [speak_summary_toggle.py](speak_summary_toggle.py) and [speak_summary_config.py](speak_summary_config.py). See [Control (Cursor slash commands only)](../README.md#control-cursor-slash-commands-only) in the repo README.
5. **Fully disable the hook:** remove the `afterAgentResponse` entry from `hooks.json` (rare; prefer `/aftertone-off`).
